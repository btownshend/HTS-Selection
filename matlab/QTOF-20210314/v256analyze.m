...
onlyc=mysql('SELECT compound FROM contents WHERE mixture=(SELECT mixture FROM mixtures WHERE name=''V5120'') ORDER BY compound');
...
cmd=sprintf('SELECT r.msrun, s.name, r.name, r.mixture, r.well, r.concentration, r.injection FROM massspec.msruns r, massspec.mssessions s WHERE r.mssession=s.mssession AND s.mssession IN (1,2,3,5)');
...
mix35=mixture(strcmp(sessionname,'2021.03.01')|strcmp(sessionname,'2021.03.14b'));
skip=strcmp(sessionname,'2021.01.31') & ismember(mixture,mix35);
