...
data={ % Well, name, filename, conc, inject
       ...
       'F1','V2560A-sn',[msdir2,'/F1.mzXML'],100e-9,10e-6,
       'G1','V2560B-sn',[msdir2,'/G1.mzXML'],100e-9,10e-6,
       'H1','V2560A-pre',[msdir2,'/H1.mzXML'],100e-9,10e-6,
       'A2','V2560B-pre',[msdir2,'/A2.mzXML'],100e-9,10e-6,
...
data=cell2struct(data,{'well','name','filename','conc','inject'},2);
for i=1:size(data,1)
...
for i=1:size(data,1)
  ...
  contains=s7contains(qsetup,s7vecs,data(i).name);
  % Save record in qsetup
  qsetup.addMS(mzdata{i},'group',n{1},'map',struct('mz',data(i).mzmap,'time',data(i).timemap),'contains',contains,'sample',data(i).name);
